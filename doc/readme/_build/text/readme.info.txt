...
This command allows retrieving many useful informations about
repository and/or developer commits.
...
As input parameter, user can specify:
   ...
   * Two commits
     ...
     Any actions requires just 2 commits. In these cases, --upstream
     and --downstream does not define a range, instead thy provide a
     way to input just that 2 commits.
   ...
   * List of commits by user:
     ...
     This option can be used with --grep/--igrep option to search only
     commit for only one user. It can fast the search on big
     repositories
   ...
   * File filter
     ...
     If you append a file name at the end of the command, action will
     act only over commits modified that file
...
---------
   ...
   Makes a difference with /NEW/BRANCH.This shows is all work done on a topic branch.
