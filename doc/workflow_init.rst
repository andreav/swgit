...
-----------------------------------
  ...
  empty or with some contents, enter the directory and issue this command::
  ...
  This happened:
    ...
    #. First, a `git init` is issued, creating .git directory
  
    #. Then, a `swgit init` is done, creating .swgit directory
  
    #. *INT/develop* and *INT/stable* branches are created

    #. *LIV/Drop.A* label has been created and checked-out
...
............
  ...
  1. You can change user owning INT branches::
     ...
     That user will also be configured into local repo, inside user.name config variable.
  
  ..

  2. You can slightly change develop and stable names::
  ...
  3. You can create a starting LIV label with the command::
     ...
     For more informations about these labels, please refer to :doc:`stabilizing`

     .. note::
       If you want to modify LIV regexp (for instance allowing any 15 letters name),
       fastest solution is:

         swgit config swgit.LIV.regexp '^[a-zA-Z]{0,15}$'
       
       For a complete reference, please refer to :doc:`custom_tags`.

  ..

  4. --share option is supported. This is forwarded to `git init`.
...
--------------------------
  ...
  When you want to create more product lines inside the same repository.
...
This is accomplished in the same manner as initializing a new swgit repository.
